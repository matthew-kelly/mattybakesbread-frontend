<footer class="relative flex flex-col items-center p-4 md:py-8 md:px-16 bg-white">
  <div class="flex justify-between items-center w-full md:mb-1">
    <a class="text-mobile-nav-logo md:text-button-lg font-semibold my-3 w-20 md:w-auto" href="/">
      Matty Bakes Bread
    </a>
    <div class="flex justify-between">
      <a class="mx-3 md:mx-8" href="/shop">Shop</a>
      <a class="mx-3 md:mx-8" href="/content">Content</a>
      <a class="ml-3 md:ml-8" href="/account">Account</a>
    </div>
  </div>
  <span
    class="absolute md:relative bottom-4 md:bottom-auto right-4 md:right-auto text-xs md:text-base text-right md:text-center"
  >
    Made with ❤️ & [SVG BREAD] by <a href="https://matthewkelly.ca" target="_blank" rel="external">
      Matty
    </a>
  </span>
</footer>
