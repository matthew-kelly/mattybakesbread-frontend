...
<div class="card w-full p-0 flex flex-col {disabled ? 'opacity-60' : ''}">
  <a href={`/shop/${product.slug.current}`} class="contents">
    <img data-src={url} src={lowQuality} alt={product.image.alt} class="w-full aspect-square lazyload" />
    <div class="flex justify-between items-center p-4 flex-grow">
      ...
      <a class="btn {disabled ? 'disabled px-3 whitespace-nowrap' : ''}" href="/shop/900g-sourdough-batard">
